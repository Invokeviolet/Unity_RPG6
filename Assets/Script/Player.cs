...
public class Player : MonoBehaviour
{
    ...
    private int PLAYERCURHP { get; set; }
    private int PLAYERMAXHP = 100;
    private int PLAYERCUREXP { get; set; }
    private int PLAYERMAXEXP = 1000;
    public int PLAYERATTACKPOWER { get; set; }
    private int PLAYERATTACKRANGE { get; set; }
    private int POTION { get; set; }
    ...
    private void OnTriggerEnter(Collider collider)
    {
        if (collider.gameObject.TryGetComponent<MonsterMarble>(out monsterMarble) == true) //(collision.collider.CompareTag("Monster"))
        {
            ...
            //this.transform.position = ActionPlayerPos.transform.position;
